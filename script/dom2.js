...
//클래스, 태그 대상을 정확히 어떤 대상만 변수로 저장하고 싶을 때
const aTag1 = document.getElementsByTagName('a');
/* 
변수 생성 시 인덱스를 작성하지 않으면 호출할 때 인덱스를 작성하며 자유롭게 원하는 대상을 가져올 수 있다.
*/
const aTag2 = document.getElementsByTagName('a')[0];//인덱스 작성버젼
/* 
변수 선언 시 인덱스를 작성하고 선언했다면 변수 사용 시 인덱스를 붙이지 않고 변수명만 간편하게 쓸 수 있다. 대신, 인덱스가 고정되므로 다른 인덱스번호에 해당하는 태그나 클래스를 사용하고 싶을 때 변수를 새로 만들어야 한다.
*/
...
//querySelector는 tag, class를 선택 시 메서드 괄호안 문자열 대상 기준으로 가장 먼저 인식된 대상을 출력해준다.
...
function btn_func2(){
    //#num 입력칸에 숫자2를 입력했을 때
    //주문 버튼을 클릭하면
    //+1 이벤트로 숫자 3이 #num 입력칸에 입력되는 코드작성
    /* console.log(Number(input_num.value))
    num_event = Number(input_num.value) + 1;
...
function btn_func3(){
    ...
    //"사용자 요청사항은 0000입니다."
    let user_answer = input_text.value
    alert(`사용자 요청사항은 ${user_answer}입니다.`)
}
function btn_func4(){
    //사용자 요청사항과 주문수량을 확인하고
    //주문 버튼을 클릭하면 다음과 같이 경고창 출력하기
    //"요청사항은 oo, 주문수량은 0개, 총 결제금액은 00원 입니다."
    //제품 1개당 1000원 기준
    let user_answer = input_text.value
    let total = Number(input_num.value)
    let price = 1000 * total
    alert(`요청사항은 ${user_answer}, 주문수량은 ${total}개, 총 결제금액은 ${price}원 입니다.`)
}