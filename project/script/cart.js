...
const original_price = document.querySelector(`del`)
const total_price = document.querySelector(`.total_price`)
let count = 1 //수량변수
let price = 1000; //가격변수
let sale = 0.10; //할인율
let sale_2 = 1-sale
total_price.innerHTML = `${price*sale_2.toLocaleString
    ('ko-kr')}원`
console.log(countInput)
countInput.value = count
original_price.innerHTML = `${price.toLocaleString
('ko-kr')}원`


//2. plus 버튼 클릭 시 count가 1씩 증가
/* const plusBtn = document.querySelector(`#num_plus`)
console.log(plusBtn)
...
plusBtn.addEventListener('click',()=>{
    ...
    count < 10 ? count++ : console.log('최대 구매수량입니다.')
    countInput.value = count
})

//3. minus 버튼 클릭 시 1씩 감소(최소 구매 수량 1)
const minusBtn = document.querySelector(`#num_minus`)
console.log(minusBtn)

minusBtn.addEventListener('click',function(){
    console.log(`-----`)
    count > 1 ? count-- : console.log('최소 구매수량입니다.')
    countInput.value = count
}) */
const plusBtn = document.querySelector(`#num_plus`)
const minusBtn = document.querySelector(`#num_minus`)
...
const plusFunc = () => {
    count < 5 ? count++ : console.log('최대 구매수량입니다.')
    countInput.value = count
    console.log(price*count)
    original_price_func()
    sale_price_func()
...
function sale_price_func(){
    let sale_price = price*count*sale_2
    total_price.innerHTML = `${sale_price.toLocaleString(`ko-kr`)}원`
}
